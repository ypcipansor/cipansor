...
removed outright rather than re-typed; the blast radius reaches 231 dependent
tables (232 including `units`), at a maximum depth of 3 — reproduced from the
post-drop FK catalog, seeded with the `SET NULL` children the migration purges by
row (the NULL-means-global tables, including `alumni_events`, plus the
catalog-matched unique-per-unit tables), in the migration header. `users` and `user_role_assignments` are not in
