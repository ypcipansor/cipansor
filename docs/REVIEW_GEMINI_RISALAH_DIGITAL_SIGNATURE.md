...
| Verifikasi keutuhan (integrity) | `POST /esign/verify-pdf` (publik, dibatasi rate, di belakang Turnstile) + verifikasi berbasis token/QR; `utils/letter-verification.ts`; checksum `documents`-style + signer list |
