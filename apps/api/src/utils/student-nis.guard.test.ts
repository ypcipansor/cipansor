...
const DOKUMEN: Array<[string, RegExp, RegExp]> = [
  ['modules/assessment/raport-merdeka.service.ts', LEWAT_UTIL, /siswa:\s*\{[^}]*nis:\s*student\.nis/],
  // Rapor terpadu memakai NIS yang sudah dihitung rapor Merdeka untuk rombelnya.
  ['modules/assessment/unified-raport.service.ts', /nis:\s*raportMerdeka\.siswa\.nis/, /nis:\s*student\.nis/],
  ['modules/rapor-pesantren/rapor-pesantren.service.ts', LEWAT_UTIL, /studentNis:\s*\w+(\.student)?\.nis\b|nis:\s*rapor\.student\.nis/],
  ['modules/emis/emis.service.ts', LEWAT_UTIL, /nis(Lokal)?:\s*student\.nis/],
  ['modules/students/id-card.service.ts', LEWAT_UTIL, /generateCardNumber\(student\.nis|generateQRCodeData\(\{[^}]*nis:\s*student\.nis/],
];
