...
// The update path now participates in the blob-claim protocol (flag 9): every
// incoming photo URL is claimed before any row references it. The protocol has
...
const tx = {
  dailyStudentReport: { update: vi.fn() },
  dailyReportPhoto: { findMany: vi.fn(), deleteMany: vi.fn(), createMany: vi.fn() },
  dailyHomework: { findMany: vi.fn(), deleteMany: vi.fn(), createMany: vi.fn() },
};
...
describe('dailyReportService.update photo replacement', () => {
  beforeEach(() => {
    ...
    tx.dailyHomework.findMany.mockResolvedValue([]);
    tx.dailyHomework.deleteMany.mockResolvedValue({ count: 0 });
    tx.dailyHomework.createMany.mockResolvedValue({ count: 1 });
...
/**
 * Homework replacement must be atomic with the report update (BUG 11) and the
 * response must carry the state AFTER the replacement (BUG 12).
 */
describe('dailyReportService.update homework replacement', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    (prisma.$transaction as any).mockImplementation(async (fn: any) => fn(tx));
    tx.dailyStudentReport.update.mockResolvedValue({ id: 'r1' });
    tx.dailyReportPhoto.findMany.mockResolvedValue([]);
    tx.dailyReportPhoto.deleteMany.mockResolvedValue({ count: 0 });
    tx.dailyReportPhoto.createMany.mockResolvedValue({ count: 0 });
    tx.dailyHomework.findMany.mockResolvedValue([]);
    tx.dailyHomework.deleteMany.mockResolvedValue({ count: 2 });
    tx.dailyHomework.createMany.mockResolvedValue({ count: 1 });
  });

  it('runs the homework delete and insert inside the same transaction as the report update (BUG 11)', async () => {
    await dailyReportService.update(
      'r1',
    ...
    expect(tx.dailyStudentReport.update).toHaveBeenCalledTimes(1);
    expect(tx.dailyHomework.deleteMany).toHaveBeenCalledWith({ where: { reportId: 'r1' } });
    expect(tx.dailyHomework.createMany).toHaveBeenCalledTimes(1);
    // The non-transactional client must not touch the homework rows.
    expect(prisma.dailyHomework.deleteMany).not.toHaveBeenCalled();
    expect(prisma.dailyHomework.createMany).not.toHaveBeenCalled();
  });

  it('keeps the previous homework when the new insert fails (BUG 11 rollback)', async () => {
    tx.dailyHomework.createMany.mockRejectedValue(new Error('insert failed'));
    ...
    // old rows are still there — nothing was actually removed.
    expect(prisma.dailyHomework.deleteMany).not.toHaveBeenCalled();
  });

  it('returns the photos and homework that exist AFTER replacement (BUG 12)', async () => {
    tx.dailyHomework.findMany.mockResolvedValue([
      { id: 'h2', reportId: 'r1', subjectName: 'Bahasa' },
    ]);
    tx.dailyReportPhoto.findMany.mockResolvedValue([
      { id: 'p2', reportId: 'r1', photoUrl: '/uploads/new.png' },
    ]);
    // The initial report read carries the OLD photos; the response must not.
    tx.dailyStudentReport.update.mockResolvedValue({
      id: 'r1',
      photos: [{ id: 'p1', photoUrl: '/uploads/old.png' }],
      homework: [],
    });
    ...
    expect(result.homework).toEqual([{ id: 'h2', reportId: 'r1', subjectName: 'Bahasa' }]);
    // The stale pre-replacement snapshot is gone.
    expect(JSON.stringify(result)).not.toContain('/uploads/old.png');
  });
});
