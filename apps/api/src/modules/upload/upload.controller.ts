...
export const uploadController = {
  uploadFile: asyncHandler(async (req: Request, res: Response) => {
    ...
    if (req.body.fileUrl) {
      ...
      // No `fileUrl` from the middleware (a mount that does not use
      // `handleSingleUpload`). Emit the same host-independent reference: an
      // absolute URL built from `req.get('host')` would break on the next
      // domain/proxy change.
      stableUrl = `/uploads/${req.file.filename}`;
    }
   ...
   */
  getSasUrl: asyncHandler(async (req: Request, res: Response) => {
