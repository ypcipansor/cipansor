...
test.describe("End-to-End: Rapor Ganda (Unified Raport) Generation", () => {
  test("should load dropdowns, select a student, and generate the unified raport", async ({
    ...
    }>(session, "GET", "/academic-years?isActive=true");
    const year = years.data[0];
    const classes = await apiRequest<{
      data: Array<{ id: string; name: string }>;
    }>(session, "GET", "/classes?limit=50");
      ...
      for (const s of (students.data as Array<any>) ?? []) {
        const raport = await apiRequest<{
          data: {
            academic?: {
              intrakurikuler?: {
                kelompokUmum?: Array<{ subjectName: string }>;
              };
            };
        ...
        const subject =
          raport?.data?.academic?.intrakurikuler?.kelompokUmum?.[0]
            ?.subjectName;
        if (subject) {
          picked = {
            className: cls.name,
    ...
    expect(
      picked,
      "seed should provide a student with a unified raport",
    ).toBeTruthy();
    if (!picked) return;

    await page.goto("/assessment/unified-raport");
    await expect(
      page.getByRole("heading", { name: /Unified SD IT Raport/i }),
    ).toBeVisible();
    ...
    await pickOption(2, `${picked.studentName} (${picked.nisn})`);
    ...
    await expect(
      page.getByText(new RegExp(`Nama:\\s*${picked.studentName}`, "i")),
    ).toBeVisible();

    // Academic section from the real aggregator
    await expect(
      page.getByRole("cell", { name: picked.subjectName }).first(),
    ).toBeVisible();
