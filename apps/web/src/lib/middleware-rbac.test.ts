...
describe("middleware rejects a forged or stale routing cookie", () => {
  ...
  it("serves /login without a routing cookie so a dead session can recover", async () => {
    // Finding 3's remedy lives on the server: a failed refresh clears the
    // `HttpOnly` routing cookie in the 401 response itself, and `/auth/session/clear`
    // does the same on the client's behalf. Once the cookie is gone the
    // middleware must serve the login form rather than redirect it away, which
    // is what lets a user with a dead session sign back in. Regression guard:
    // if `/login` redirected here, the loop would return.
  ...
  it("routes an authenticated /login to a dashboard while the routing cookie is valid", async () => {
    const res = await middleware(
      request("/login", {
        [ROUTING_COOKIE]: await routingCookieFor(
          "UNIT_ADMIN",
