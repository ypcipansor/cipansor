...
function UnifiedRaportPageContent({
  ...
  const {
    ...
    isLoading,
    error,
  } = useUnifiedRaport(params.studentId, academicYearId, semester);
    ...
    return (
      <div className="container mx-auto py-8 text-center">
        <p className="text-destructive">
          Parameter tidak lengkap. Pastikan academicYearId dan semester tersedia
          di URL.
        </p>
      </div>
    );
  ...
  if (error || !raport) {
    return (
      <div className="container mx-auto py-8 text-center">
        <p className="text-destructive">
          Gagal memuat data rapor. Pastikan parameter sudah benar.
        </p>
      </div>
    );
  }
    ...
    <div className="container mx-auto py-8 space-y-8 max-w-5xl">
          ...
          <div className="mt-6 grid grid-cols-2 text-left gap-4 max-w-2xl mx-auto border p-4 rounded-lg">
            ...
            <div>
              ...
              <p className="font-semibold">
                {raport.student.nis} / {raport.student.nisn || "-"}
              </p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground uppercase font-bold">
        ...
        <CardContent className="pt-6 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-3">
              ...
              <p className="text-sm text-muted-foreground">
                Hafalan Terakhir: <b>{raport.islamic.tahfidz.latestSurah}</b>
              </p>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between items-center bg-slate-50 p-3 rounded">
                ...
                >
                  {raport.islamic.grade}
                </Badge>
              </div>
              <p className="text-sm text-muted-foreground">
                Skor Kedisiplinan: <b>{raport.islamic.score}</b>
              </p>
            </div>
          </div>
        </CardContent>
      ...
      {raport.remarks.recommendation && (
        <Alert className="bg-purple-50 border-purple-200">
          ...
          <AlertTitle className="text-purple-800 font-bold flex items-center gap-2">
            ...
            <Badge
              variant="outline"
              className="text-[10px] bg-white text-purple-600 border-purple-200"
            >
              Experimental
            </Badge>
          </AlertTitle>
        ...
        <Card className="border-l-4 border-l-blue-500 bg-blue-50/30">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-bold text-blue-800 flex items-center gap-2">
              <TrendingUp className="h-4 w-4" /> Analisis Holistik (Kepribadian
              & Akademik)
            </CardTitle>
          </CardHeader>
          <CardContent>
          ...
          <CardContent>
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                <span>Sakit</span>
                <span>{raport.academic.attendance?.sakit ?? 0} hari</span>
              </div>
              <div className="flex justify-between text-sm">
                <span>Izin</span>
                <span>{raport.academic.attendance?.izin ?? 0} hari</span>
              </div>
              <div className="flex justify-between text-sm">
                <span>Tanpa Keterangan</span>
                <span>{raport.academic.attendance?.alpa ?? 0} hari</span>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="mt-16 grid grid-cols-3 text-center gap-4 text-sm pb-12">
        <div>
          ...
          <p className="font-bold border-b border-black inline-block px-8">
        ...
        <div>
          ...
          <div className="h-24"></div>
          <p className="font-bold border-b border-black inline-block px-8">
            {raport.signatures.homeroomTeacher || "NAMA GURU"}
          </p>
        </div>
        <div>
          <p>
...
export default function UnifiedRaportPage(
  props: Parameters<typeof UnifiedRaportPageContent>[0],
) {
  return (
