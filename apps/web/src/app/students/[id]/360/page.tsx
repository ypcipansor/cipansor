...
import {
  useStudentRoomAssignment,
  useRoomSocialAnalytics,
...
function Student360PageContent() {
  ...
  const { data: health, isLoading: loadingHealth } =
    useStudentHealthRecords(studentId);
  const { data: violations, isLoading: loadingViolations } =
    useStudentViolationSummary(studentId);
  const { data: finance, isLoading: loadingFinance } =
    useStudentFinancialSummary(studentId);
  const { data: tahfidz, isLoading: loadingTahfidz } =
    useStudentTahfidzProgress(studentId);
  const { data: roomAssignment, isLoading: loadingRoom } =
    useStudentRoomAssignment(studentId);
  const { data: socialAnalytics, isLoading: loadingSocial } =
    useRoomSocialAnalytics(roomAssignment?.roomId || "");

  // Fallback to active academic year when student.currentClass.academicYear is not populated
  const { data: activeYear } = useActiveAcademicYear();
  const academicYearId =
    student?.currentClass?.academicYear?.id || activeYear?.id || "";

  // New Education Enhancement Hooks
  const { data: holistic, isLoading: loadingHolistic } =
    useStudentHolisticAnalytics(studentId, academicYearId);
  const { data: ibadah, isLoading: loadingIbadah } = useStudentIbadahStats({
    studentId,
  });
        ...
        <div className="grid grid-cols-2 md:flex gap-3 w-full md:w-auto">
          <Card className="flex-1 md:w-32 bg-slate-50 border-none shadow-none">
            <CardContent className="p-3 text-center">
              ...
              <p
                className={`text-xl font-bold mt-1 ${violationTotalPoints > 50 ? "text-rose-600" : "text-slate-900"}`}
              >
          ...
          <Card className="flex-1 md:w-32 bg-slate-50 border-none shadow-none">
            <CardContent className="p-3 text-center">
              ...
              <p className="text-xl font-bold mt-1 text-indigo-600">
                {tahfidz?.summary?.juzCoveredCount || 0}
              </p>
            </CardContent>
          </Card>
        </div>
            ...
            <CardContent className="text-sm space-y-3">
              ...
              <div className="pt-3 border-t">
                <div className="flex justify-between items-center mb-1">
                  ...
                  <Badge className="bg-emerald-50 text-emerald-700 text-[10px] h-5">
                    SEHAT
                  </Badge>
                </div>
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xs font-bold">
                    Rp{" "}
                    {Number(finance?.totalOutstanding || 0).toLocaleString(
                      "id-ID",
                    )}
                  </span>
                </div>
                ...
                <div>
                  <div className="flex justify-between text-xs mb-1">
                    ...
                    <span className="font-bold">
                      {holistic?.breakdown?.academic ?? "—"}
                    </span>
                  </div>
                  <Progress
                    value={holistic?.breakdown?.academic ?? 0}
                ...
                <div className="grid grid-cols-2 gap-2 text-center pt-2">
                  <div className="bg-blue-500/50 rounded-lg p-2">
                    ...
                    <p className="text-lg font-bold">
                      {holistic?.breakdown?.attendance != null
                        ? `${Math.round(holistic.breakdown.attendance)}%`
                        : "—"}
                    </p>
                  </div>
                  <div className="bg-blue-500/50 rounded-lg p-2">
            ...
            <TabsContent value="overview" className="space-y-6">
                  ...
                  <CardHeader className="pb-2">
                    ...
                    <CardDescription className="text-indigo-700 font-medium flex items-center gap-2">
                      ...
                      {holistic.dataCompleteness === "INSUFFICIENT" && (
                        <Badge
                          variant="outline"
                          className="text-rose-600 border-rose-300 text-[10px]"
                        >
                          Data Tidak Cukup
                        </Badge>
                      )}
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          ...
                          <div className="text-center p-2 rounded-lg bg-emerald-100/50">
                            ...
                            <p className="text-xl font-black text-emerald-900">
                              {holistic.dataCompleteness === "INSUFFICIENT"
                                ? "—"
                                : holistic.holisticScore >= 80
                                  ? "Tinggi"
                                  : holistic.holisticScore >= 60
                                    ? "Sedang"
                                    : "Perlu Perhatian"}
                            </p>
                          </div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-emerald-600">
                            {student.bloodType ? "Tercatat" : "Belum tercatat"}
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="p-3 border rounded-lg bg-slate-50">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {student.specialNeeds ||
                          "Tidak ada data alergi tercatat"}
                      </p>
                    </div>
                  </CardContent>
                </Card>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    {recentViolations.length > 0 ? (
                      <div className="divide-y">
                        {recentViolations.slice(0, 3).map((v: any) => (
                          <div
                            key={v.id}
                            className="p-3 px-6 flex justify-between items-center text-sm"
                          >
                            <div>
                ...
                <Card className="border-l-4 border-l-blue-500">
                  <CardHeader className="pb-2">
                    <CardTitle className="text-base flex items-center gap-2">
                      <Wallet className="w-4 h-4 text-blue-500" /> Status
                      Keuangan
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="p-6">
                    ...
                    <Button
                      ...
                      asChild
                    >
                      <a href={`/finance/invoices?studentId=${studentId}`}>
                        Lihat Semua Tagihan
                      </a>
                    </Button>
                  </CardContent>
              ...
              <Card>
                ...
                <CardContent>
                  <p className="text-sm text-muted-foreground py-10 text-center">
                    Modul Akademik Lengkap dalam Pengembangan.
                  </p>
                </CardContent>
              </Card>
            </TabsContent>
                ...
                <CardContent className="space-y-6">
                  <div className="flex justify-between items-end">
                    ...
                    <div className="text-right">
                      ...
                      <Progress
                        value={
                          ((tahfidz?.summary?.juzCoveredCount || 0) / 30) * 100
                        }
                          ...
                          {latest ? (
                            <>
                              <p className="text-sm font-bold">
                                {latest.surahName}: {latest.ayahStart}-
                                {latest.ayahEnd} (Juz {latest.juz})
                              </p>
                  ...
                  {tahfidz?.estimation && (
                    <div className="p-4 border rounded-xl bg-indigo-50/50 border-indigo-100">
                      <h5 className="text-xs font-bold uppercase mb-2 text-indigo-600">
                        Estimasi Khatam 30 Juz
                      </h5>
                      {tahfidz.estimation.status === "COMPLETED" ? (
                        <p className="text-sm font-bold text-emerald-700">
                          Alhamdulillah, hafalan 30 juz telah selesai.
                        </p>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
                          <div>
                            ...
                            <p className="text-[10px] text-muted-foreground">
                              Perkiraan selesai (±{" "}
                              {tahfidz.estimation.estimatedDays} hari)
                            </p>
                          </div>
                          <div>
                            <p className="font-bold">
                              {tahfidz.estimation.ayahPerDay} ayat/hari
                            </p>
                            <p className="text-[10px] text-muted-foreground">
                              Kecepatan 90 hari terakhir
                            </p>
                          </div>
                          <div>
                            <p className="font-bold">
                              {tahfidz.estimation.remainingAyah} ayat
                            </p>
                            <p className="text-[10px] text-muted-foreground">
                              Sisa menuju 6.236 ayat
                            </p>
                          </div>
                        </div>
                      ) : (
                        <p className="text-xs italic text-muted-foreground">
                          Belum cukup data setoran (minimal 3 catatan ziyadah
                          dalam 90 hari terakhir) untuk membuat proyeksi.
                        </p>
                      )}
                    </div>
                  )}
            ...
            <TabsContent value="behavior">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Monitoring point dan catatan wali kelas
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <p className="text-sm text-muted-foreground py-10 text-center">
                    Data histori kedisiplinan lengkap dapat dilihat di tab
                    Overview atau menu Pelanggaran.
                  </p>
                </CardContent>
              </Card>
            </TabsContent>
            ...
            <TabsContent value="counseling">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Catatan perkembangan psikologis dan pembinaan
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <p className="text-sm text-muted-foreground py-10 text-center">
                    Modul Bimbingan & Konseling dalam Pengembangan.
                  </p>
                </CardContent>
              </Card>
            </TabsContent>
              ...
              {loadingRoom || (roomAssignment?.roomId && loadingSocial) ? (
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Informasi kamar dan dinamika sosial
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <p className="text-sm text-muted-foreground py-10 text-center">
                      Santri belum memiliki penempatan kamar.
                    </p>
                  </CardContent>
                </Card>
              ) : (
                    ...
                    <CardContent className="space-y-4">
                      <div className="p-4 bg-slate-50 rounded-xl border">
                        ...
                        <p className="text-xs text-slate-500">
                          {roomAssignment.room?.dormitory?.name} • Lt.{" "}
                          {roomAssignment.room?.floor}
                        </p>
                      </div>
                      <div className="grid grid-cols-2 gap-2">
                        <div className="p-3 border rounded-lg text-center">
                          ...
                          <p className="text-lg font-bold">
                            {roomAssignment.room?.capacity} Bed
                          </p>
                        </div>
                        <div className="p-3 border rounded-lg text-center">
                          <p className="text-[10px] text-muted-foreground uppercase">
                          ...
                          <p className="text-lg font-bold">
                            {roomAssignment.room?.currentOccupancy} Santri
                          </p>
                        </div>
                      </div>
                    </CardContent>
                  ...
                  <Card className="md:col-span-2 border-indigo-100 bg-indigo-50/30">
                    <CardHeader className="pb-2">
                      ...
                      <CardDescription className="text-indigo-700">
                        Status: {socialAnalytics?.status || "NORMAL"}
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                      <div className="flex flex-col md:flex-row gap-6 items-center">
                        <div className="relative h-32 w-32 flex items-center justify-center">
                          <svg className="w-full h-full transform -rotate-90">
                            ...
                            <circle
                              ...
                              strokeDashoffset={
                                364.4 -
                                (364.4 * (socialAnalytics?.harmonyScore || 0)) /
                                  100
                              }
                          ...
                          <div className="absolute inset-0 flex flex-col items-center justify-center">
                            <span className="text-2xl font-black text-indigo-900">
                              {socialAnalytics?.harmonyScore || 0}%
                            </span>
                        ...
                        <div className="flex-1 space-y-3">
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              ...
                              <p className="text-lg font-bold text-slate-700 mt-1">
                                {socialAnalytics?.members?.length || 0} Santri
                              </p>
                            </div>
                            <div>
                              <p className="text-xs font-bold text-slate-500 uppercase">
                          ...
                          <div className="p-3 bg-white rounded-lg border border-indigo-100 text-xs">
                            <p className="font-bold text-indigo-900 mb-1">
                              Teman Sekamar (
                              {socialAnalytics?.members?.length || 0}):
                            </p>
            ...
            <TabsContent value="health">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Histori kunjungan ke unit kesehatan
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    {Array.isArray(health) && health.length > 0 ? (
                        ...
                        >
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground mt-1">
                              Diagnosis: {h.diagnosis || "Dalam Observasi"}
                            </p>
                      ...
                      <p className="text-sm text-muted-foreground py-10 text-center">
                        Tidak ada riwayat kunjungan medis.
                      </p>
                    )}
                  </div>
                </CardContent>
